    ...
    public class ExpectingSerialPort
    {
        ...
        private void myPort_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            lock (expectedLock)
            {
                while (myPort.BytesToRead > 0)
                {
                    ...
                    Debug.WriteLine(String.Format("<- 0x{0:X2} {0} '{1}' E", got, (( got == 0) ? "\\0" : ((char)got + ""))));
                    if (exp != got)
                    {
        ...
        public void Write(byte[] buffer, int offset, int count)
        {
            ...
            foreach (byte b in buffer)
                Debug.WriteLine(String.Format("-> 0x{0:X2} {0} '{1}'", b, ((b == 0) ? "\\0" : ((char)b + ""))));
        ...
        public void Read(byte[] buffer, int offset, int length)
        {
            ...
            {
                Debug.WriteLine(String.Format("<- 0x{0:X2} {0} '{1}' R", buffer[i], ((buffer[i] == 0) ? "\\0" : ((char)buffer[i] + ""))));
            }
        }
        ...
        internal void FlushExpected() {
            //Sleep until all expected bytes are flushed
            expected.Enqueue(MAGIC_WAIT_CONSTANT);
            while (expected.Peek() != MAGIC_WAIT_CONSTANT)
                sleep();
            expected.Dequeue();
